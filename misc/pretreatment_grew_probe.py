...
parser.add_argument('--worker_num', default=1, type=int,
                    help='How many subprocesses to use for data pretreatment. '
                         'Default: 1')
parser.add_argument('--img_h', default=64, type=int,
                    help='image size')
...
if SUBSET != "test/probe":
    raise ValueError("subset must be test/probe")
...
def cut_pickle(seq_info, pid, subset):
    seq_name = '-'.join(seq_info)
    log_print(pid, START, seq_name)
    seq_path = os.path.join(INPUT_PATH, subset, *seq_info)
    out_dir = os.path.join(OUTPUT_PATH, subset, *seq_info)
...
# Walk the input path
seq_list = os.listdir(os.path.join(INPUT_PATH, SUBSET))
seq_list.sort()
