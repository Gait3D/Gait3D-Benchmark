...
class DataSet_GREW_our(tordata.Dataset):
    ...
    def __load_seqs_into_list(self, dataset_root, lab, typ, vie, seqs_info_list, data_in_use):
        msg_mgr = get_msg_mgr()
        seq_info = [lab, typ, vie]
        seq_path = osp.join(dataset_root, *seq_info[:-1])
    ...
    def __dataset_parser(self, data_config, training):
        ...
        def get_seqs_info_list(label_set):
            ...
            for lab in label_set:
                for seq in sorted(os.listdir(osp.join(dataset_root, lab))):
                    vie = "000"
                    id_seq = f"{lab}-{seq}"
                    if id_seq in probe_set:
