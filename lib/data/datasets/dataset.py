...
class DataSet(tordata.Dataset):
    ...
    def __load_seqs_into_list(self, dataset_root, dataset_name, lab, typ, vie, seqs_info_list, data_in_use):
        msg_mgr = get_msg_mgr()
        seq_info = [lab, typ, vie]
        seq_path = osp.join(dataset_root, *seq_info)
        seq_dirs = sorted(os.listdir(seq_path))
        if "Gait3D" in dataset_name:
            cam_typ = typ.split("_videoid")[0]
            cam_id = int(cam_typ.split("camid")[1])
            seq_info = [lab, cam_id, vie]
        if seq_dirs != []:
    ...
    def __print_dataset_csv_format(self, dataset, set_info):
        """
        ...
        """
        msg_mgr = get_msg_mgr()
        headers = list(set_info.keys())
        csv_results = []
        for data_info in [set_info]:
    ...
    def __visualize_data_info(self, dataset_name, label_set, training, seqs_info_list):
        if not training:
            test_info = {
                "subset": "test",
            ...
            self.__print_dataset_csv_format(f"{dataset_name}-testset", set_info=test_info)
        if training:
            train_info = {
                "subset": "train",
                "ids": len(set(label_set)),
            ...
            self.__print_dataset_csv_format(f"{dataset_name}-trainset", set_info=train_info)
    ...
    def __dataset_parser(self, data_config, training):
        ...
        def get_seqs_info_list(label_set):
            ...
            for lab in label_set:
                for typ in sorted(os.listdir(osp.join(dataset_root, lab))):
                    for vie in sorted(os.listdir(osp.join(dataset_root, lab, typ))):
                        id_cam_seq = f"{lab}-{typ}-{vie}"
                        if id_cam_seq in probe_set:
                            self.__load_seqs_into_list(dataset_root, dataset_name,
                                                       lab, typ, vie,
            ...
            self.__visualize_data_info(dataset_name, label_set, training, seqs_info_list)
