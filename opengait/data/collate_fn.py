...
class CollateFn(object):
    def __init__(self, label_set, sample_config):
        ...
        try:
            self.uniform = sample_config['uniform']
        except:
            self.uniform = False
        try:
            self.noncyclic = sample_config['noncyclic']
        except:
            self.noncyclic = False
        ...
        if self.sampler != 'all' and self.ordered and not self.uniform and not self.noncyclic:
    ...
    def __call__(self, batch):
        ...
        def sample_frames(seqs):
            ...
            if self.sampler in ['fixed', 'unfixed']:
                ...
                if self.ordered and not self.uniform and not self.noncyclic:
                ...
                elif self.ordered and self.uniform:
                    if seq_len > frames_num:
                        step = seq_len // frames_num
                        start = np.random.randint(step)
                        end = start + frames_num * step
                        indices = indices[start: end: step]
                    else:
                        indices = indices[0:frames_num]
                        while len(indices) < frames_num:
                            for index in indices:
                                if len(indices) >= frames_num:
                                    break
                                indices.append(index)
                        indices = sorted(indices)
                elif self.ordered and self.noncyclic:
                    if seq_len > frames_num:
                        start = np.random.randint(seq_len - frames_num + 1)
                        indices = indices[start: start + frames_num]
                    else:
                        indices = indices[0:frames_num]
                        while len(indices) < frames_num:
                            for index in indices:
