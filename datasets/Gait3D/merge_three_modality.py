...
def merge(sils_path, smpls_path, pars_path, output_path, link):
    if link == 'hard':
        link_method = os.link
    elif link == 'soft':
        ...
        for _type in os.listdir(id_path):
            ...
            for _view in os.listdir(type_path):
                ...
                for _seq in os.listdir(view_path):
                    ...
                    pars_seq_path = os.path.join(
                        pars_path, _id, _type, _view, _seq)
                    ...
                    link_method(pars_seq_path, os.path.join(
                        output_seq_path, "pars-"+_seq))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pars_path', default='', type=str,
                        help='Root path of raw pars dataset.')
    ...
    merge(sils_path=Path(args.sils_path), smpls_path=Path(
        args.smpls_path, args.pars_path), output_path=Path(args.output_path), link=args.link)
